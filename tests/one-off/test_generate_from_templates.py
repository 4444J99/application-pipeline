...
def test_generation_workflow(tmp_path, monkeypatch):
    ...
    from generate_from_templates import generate_all
    
    # Patch the constants in the script
    monkeypatch.setattr("generate_from_templates.REPO_ROOT", repo_root)
    monkeypatch.setattr("generate_from_templates.TEMPLATES_DIR", repo_root / "templates")
    monkeypatch.setattr("generate_from_templates.METRICS_FILE", metrics_file)

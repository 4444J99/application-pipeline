...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../scripts/deprecated')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../../scripts')))

import verify_sources as verify_mod
from verify_sources import verify_ashby, verify_greenhouse


def test_verify_greenhouse_success(monkeypatch):
    """Verify verify_greenhouse handles successful API response."""
    mock_urlopen = MagicMock()
    monkeypatch.setattr(verify_mod, "urlopen", mock_urlopen)
...
def test_verify_greenhouse_http_error(monkeypatch):
...
def test_verify_ashby_success(monkeypatch):
...
def test_verify_ashby_decode_error(monkeypatch):
